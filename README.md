### CHOLOGBOOK Creator

작은 행동(Patch)을 쌓고,  
그 흐름(Minor)을 인지하며,  
변화(Major)를 발견하는 과정을 설계합니다.

기록을 대신해주는 것이 아니라,  
스스로 변화를 느낄 수 있는 타이밍을 만드는 시스템을 만들고 있습니다.

---

#### 활동적인 내향형 (ISFJ)

- 직접 부딪히며 경험하는 것을 좋아합니다  
- 춤을 가장 좋아합니다 🎧  
- 혼자 몰입하는 시간에서 에너지를 얻습니다  

---

#### 성장 방식

- 작은 실행을 쌓습니다  
- 흐름을 관찰합니다  
- 변화를 인지합니다  

---

### Project

#### CHOLOGBOOK | 초록북

Patch → Minor → Major 구조를 기반으로  
행동이 흐름이 되고, 흐름이 변화로 이어지도록 만드는 기록 시스템

---

### 🔗 Links

- Notion: [https://chologbook.notion.site](https://wise-tumbleweed-aa3.notion.site/33d672f9fc07802d9040cadcf37c9d44?source=copy_link)
- GitHub Repo: [https://github.com/kimfield98](https://github.com/kimfield98/kimfield98/edit/main/README.md)
- Service: [https://www.chologbook.com/](https://www.chologbook.com/)
